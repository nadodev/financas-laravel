...
@section('header')
    <div class="flex items-center justify-between">
        <h2 class="text-xl font-semibold text-gray-900">Editar Orçamento</h2>
    </div>
@endsection
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            ...
                            <input type="number" step="0.01" min="0" name="amount" id="amount" value="{{ old('amount', $budget->amount) }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="start_date" id="start_date" value="{{ old('start_date', optional($budget->start_date)->format('Y-m-d')) }}"
                        ...
                        <div>
                            ...
                            <input type="date" name="end_date" id="end_date" value="{{ old('end_date', optional($budget->end_date)->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <select id="recurrence" name="recurrence" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md @error('recurrence') border-red-300 text-red-900 placeholder-red-300 focus:outline-none focus:ring-red-500 focus:border-red-500 @enderror">
                            ...
                            <option value="monthly" {{ old('recurrence', $budget->recurrence) == 'monthly' ? 'selected' : '' }}>Mensal</option>
                            <option value="quarterly" {{ old('recurrence', $budget->recurrence) == 'quarterly' ? 'selected' : '' }}>Trimestral</option>
                            <option value="yearly" {{ old('recurrence', $budget->recurrence) == 'yearly' ? 'selected' : '' }}>Anual</option>
                        </select>
                        @error('recurrence')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                            ...
                            placeholder="Descrição opcional do orçamento">{{ old('description', $budget->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
